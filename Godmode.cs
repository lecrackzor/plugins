...
namespace Oxide.Plugins
{
    ...
    [Info("Godmode", "Mughisi", 2.0, ResourceId = 660)]
    class Godmode : RustPlugin
    {
        ...
        void OnPlayerInit(BasePlayer player)
        {
            if (gods[player.userID] != null)
            {
                ...
                if (playerPrefixEnabled)
                    player.displayName = $"{playerPrefix} {player.displayName}";
            }
        }
        ...
        {
            if (!IsAllowed(player, "CanUseGodmode")) return;
        ...
        {
            if (!permission.PermissionExists("CanUseGodmode")) permission.RegisterPermission("CanUseGodmode", this);
        }
